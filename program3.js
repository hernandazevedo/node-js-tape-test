...
var test = require("tape");
var fancify = require(process.argv[2] || './fancify.js');

test('should return ~*~Hello~*~ ',function(t){
    
    var strHello = fancify('Hello')
    t.assert(strHello == '~*~Hello~*~','OK returned the right value ~*~Hello~*~');
    t.end()
})

test('should return ~*~HELLO~*~ ',function(t){
    
    var strHello = fancify('Hello',true)
    t.assert(strHello == '~*~HELLO~*~','OK returned the right value ~*~HELLO~*~');
    t.end()
})
